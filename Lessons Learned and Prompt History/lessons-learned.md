...
| 12 | Reads worked but **creating a product failed** (`500 "Something went wrong"`; server log: `TypeError: b.mask is not a function` → `Connection terminated unexpectedly`) | A prior fix had *bundled* `ws`; bundling mangles `ws`'s internal frame-masking code, so the WebSocket the Neon adapter opens for writes that return relations (`create … include`) couldn't send frames. (Reads use HTTPS fetch, so they were unaffected — which is why only writes broke.) | Move `ws` (and the Neon packages) **back to `serverExternalPackages`** so Node loads `ws` intact; also refactored stock changes to an atomic conditional `updateMany` and split product+movement into separate inserts to minimize WebSocket use. Reproduced and fixed locally via `npm start` against the live DB. |
